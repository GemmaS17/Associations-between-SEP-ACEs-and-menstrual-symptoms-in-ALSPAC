...
setwd("/Volumes/Studies/ALSPAC Menstrual PhD")
...
library(haven)    ## loading the package
#alspac.table_ACE=read_dta("ACE_data_merged222.dta") 
# Needs to be in STATA v12/11
alspac.table_ACE=read.dta("ACEs_GSsample.dta", 
                          convert.factors = TRUE)

ls()#Should now have a dataframe called 'alspac.table_ACE' in environment
class(alspac.table_ACE)
dim(alspac.table_ACE)#5960 participants with 1020 variables. 
...
#iii. vector ALSPAC variables needed to derive household social class (16 vector)
...
#Outcomes - pain_both heavy_both days_both length_both irreg pms_bin has_pub has_length has_irreg has_pms 
#Confounders - highed sclass findiff ethnicity
#IPW - mat_age marital_stat phone car housing rooms crowding dbl_glaze first_preg smoke_preg smoke_ever epds parity mated findiff_ipw bfed_dur fai_long mat_ethnicity
#Other sensitivity - menarche mated pated mat_sclass pat_sclass pain_cont heavy_cont days_bin_cont length_cont irreg_cont pms_cont pain_tsm heavy_tsm days_bin_tsm length_tsm irreg_tsm pms_tsm pain_doc heavy_doc heavy_days_cat
 

impute_ACE<-
  c("c804", "kz030", "kz029", "dw002", "dw042", "a006", "mz028b", "b032", "a525", "c645a", "c666a", "pb325a", "pb342a", "kz021", 
    "b608", "b593", "c472", "c520", "c522", "pb183", "b370", "c600", "pb260", "b106", "b107", "b122", "b123", "b597", "c093", "c101", 
    "d152", "d169", "d170", "pa172", "pa189", "pa190", "pb187", "a600", "b598", "pb188a", "b578", "b587", "pb168", "pb177", "sc_household_18wgest", "sc_household_32wgest",
    "d790", "pb130", "b701", "b702", "b714", "d167", "d168", "pa187", "pa188", "pb098", "t3336", "t3337", "fa3333", "fa3334", "t3322", 
    "t1360", "t1362", "fa1360", "fa1362", "t5404", "t3327", "t3255", "t5300", "t5305", "t5320", "t5340", "t5345", "t5360", 
    "t5365", "t5380", "t5385", "fa5404", "t2035", "fa2035", "t3328", "fa3328", "t3308", "t3316", "fa3308", "fa3316", "t3325", 
    "t3326", "fa3325", "fa3326", "t5402", "t5406", "t5410", "t5412", "t5510", "fa5402", "fa5406", "fa5410", "fa5411", "fa5510", "ypa5005_dup", 
    "ypa5007_dup", "ypa5009_dup", "ypa5011_dup", "ypa5013_dup", "ypa5015_dup", "ypa5017_dup", "ypa5050", "fa3321","b665","b667","c482","e178",
    "c521","c523","h731","h733","h734", "c525",
    "pain_both", "heavy_both", "days_both", "length_both", "irreg", "pms_bin", "has_pub", "has_length", "has_irreg", "has_pms",  
    "highed", "sclass", "findiff", "ethnicity",
    "mat_age", "marital_stat", "phone", "car", "housing", "rooms", "crowding", "dbl_glaze", "first_preg", "smoke_preg", "smoke_ever", "epds", "parity", "mated", 
    "findiff_ipw", "bfed_dur", "fai_long", "mat_ethnicity", 
    "menarche", "mated", "pated", "mat_sclass", "pat_sclass", "pain_cont", "heavy_cont", "days_bin_cont", "length_cont", "irreg_cont", "pms_cont", 
    "pain_tsm", "heavy_tsm", "days_bin_tsm", "length_tsm", "irreg_tsm", "pms_tsm", "pain_doc", "heavy_doc", "heavy_days_cat"
    )
...
#BF: some of these missing variables have been renamed.
#BF: Going to change back to the original names & rerun the above code to check which variables are missing.
names(alspac.table_ACE)[names(alspac.table_ACE)=="FJPC4000"]<-"fjpc4000"
names(alspac.table_ACE)[names(alspac.table_ACE)=="FJPC4100"]<-"fjpc4100"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8000"]<-"ypb8000"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8001"]<-"ypb8001"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPC1812"]<-"ypc1812"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8050"]<-"ypb8050"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8051"]<-"ypb8051"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8100"]<-"ypb8100"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPC1814"]<-"ypc1814"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8120"]<-"ypb8120"
names(alspac.table_ACE)[names(alspac.table_ACE)=="FJPC2100"]<-"fjpc2100"
names(alspac.table_ACE)[names(alspac.table_ACE)=="CCXD750"]<-"ccxd750"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPC1810"]<-"ypc1810"
names(alspac.table_ACE)[names(alspac.table_ACE)=="FJPC2000"]<-"fjpc2000"
names(alspac.table_ACE)[names(alspac.table_ACE)=="FJLE112"]<-"fjle112"
names(alspac.table_ACE)[names(alspac.table_ACE)=="FJLE114"]<-"fjle114"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8002"]<-"ypb8002"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8003"]<-"ypb8003"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8006"]<-"ypb8006"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8007"]<-"ypb8007"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPC1811"]<-"ypc1811"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8052"]<-"ypb8052"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8053"]<-"ypb8053"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8056"]<-"ypb8056"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8057"]<-"ypb8057"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8030"]<-"ypb8030"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8040"]<-"ypb8040"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPC1813"]<-"ypc1813"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8080"]<-"ypb8080"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPB8090"]<-"ypb8090"
names(alspac.table_ACE)[names(alspac.table_ACE)=="FJPC100"]<-"fjpc100"
names(alspac.table_ACE)[names(alspac.table_ACE)=="FJPC150"]<-"fjpc150"
names(alspac.table_ACE)[names(alspac.table_ACE)=="FJPC200"]<-"fjpc200"
names(alspac.table_ACE)[names(alspac.table_ACE)=="FJPC050"]<-"fjpc050"
names(alspac.table_ACE)[names(alspac.table_ACE)=="FJAL4000"]<-"fjal4000"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5005"]<-"ypa5005"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5007"]<-"ypa5007"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5009"]<-"ypa5009"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5011"]<-"ypa5011"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5013"]<-"ypa5013"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5015"]<-"ypa5015"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5017"]<-"ypa5017"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5050"]<-"ypa5050"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5004"]<-"ypa5004"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5006"]<-"ypa5006"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5008"]<-"ypa5008"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5010"]<-"ypa5010"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5012"]<-"ypa5012"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5014"]<-"ypa5014"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5016"]<-"ypa5016"
names(alspac.table_ACE)[names(alspac.table_ACE)=="YPA5050"]<-"ypa5050"
#still some missing:"fa3322", "fa3327". I didn't find these, I don't need these anyway so I excluded them from the excel sheet.

# Check again if you have all the variables you need, should be TRUE
all(check_in_alspac_data%in%names(alspac.table_ACE))
...
##alspac.table_ACE$sc_household_12wgest<-
 ## factor(sc_household_numeric=character("pa_sc_p"),levels,labels=levels)
...
######################
class(alspac.table_ACE$c482)
table(alspac.table_ACE$c482)
alspac.table_ACE$c482[alspac.table_ACE$c482 == -1] <- NA
alspac.table_ACE$c482[alspac.table_ACE$c482 == -7] <- NA
table(alspac.table_ACE$c482)

class(alspac.table_ACE$e178)
table(alspac.table_ACE$e178 )
alspac.table_ACE$e178 <- as.character(alspac.table_ACE$e178)
alspac.table_ACE$e178[alspac.table_ACE$e178== "-1"] <- NA
alspac.table_ACE$e178 <- as.factor(alspac.table_ACE$e178)
table(alspac.table_ACE$e178 )
...
#sapply(factors[1],function(x) factors[1][x]%in%names(alspac.table_ACE))
...
## BF: some of the variable labels in the data were not consistent with the ACE worksheet labels,
#I've added some code in this loop that stops the loop and prints the variable where the error occurs 
...
table(paste0(impute_ACE,'_org')%in%names(binary_alspacKids_ACE_data))
#should be TRUE for all 186 variables
...
save(binary_alspacKids_ACE_data,file= paste0 (loc_out, "binary_alspacKids_ACE_data_ALL.RData"))
