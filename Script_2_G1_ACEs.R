...
#Changed the loc_inp and loc_out to own folders.
setwd(loc_inp)
setwd(loc_out)
...
#   Spelling is case sensitive
name_data<-'binary_alspacKids_ACE_data_ALL.RData'
...
timeperiod<-c(0,10)#Default 0-16 yrs, which means 
...
threshold_proportion_answered_ACE0_10<-0.1#numerical value between 0-1
...
dim(binary_alspacKids_ACE_data)  #5960 observations for 892 variables
...
#A. Specify time period
if(timeperiod[1]==0 & timeperiod[2]==10){#default
...
#C.  Exclude participants based on threshold nr ACE questions answered - threshold of 0.1 (10%); drops sample down to 5567
ACE_var0_10<-unique(tolower(adv_description$variable[
#making sure it's questions answered by using variable NOT variable_unique
which(adv_description$time_yrs1>=0 &
        adv_description$time_yrs2<=10) 
]))
jpeg('hist_prop_answeredACE.jpg')
hist(abs((rowSums(is.na(binary_alspacKids_ACE_data[,ACE_var0_10]))/length(ACE_var0_10))-1),
     main='',xlab='Proportion answered ACE questions',ylab='Number of individuals')
dev.off()

sum(!(rowSums(is.na(binary_alspacKids_ACE_data[,ACE_var0_10]))/
        length(ACE_var0_10))>=(1-threshold_proportion_answered_ACE0_10))
binary_alspacKids_ACE_data<-
  binary_alspacKids_ACE_data[
    !(rowSums(is.na(binary_alspacKids_ACE_data[,ACE_var0_10]))/
        length(ACE_var0_10))>=(1-threshold_proportion_answered_ACE0_10),]
...
rm(exclude_var,exclude_sc,exclude_twin,sample_Boyd,
   ACE_var0_10,threshold_proportion_answered_ACE0_10)
...
ACEmeasures<-#ACEs in correct order
  c("ACEscore_classic", "ACEcat_classic",
    ...
    "bullying","violence_between_parents",
    "substance_household", 
...
ACE_time1_time2<-data.frame(
  sapply(ACEs,function(ACE,ACE_var,...){
    ...
    if(length(vars_ACE)==0){catf('\n\nNo data available on', ACE,
      ...
      print(dim(binary_alspacKids_ACE_data[,quest_ACE]))  # Add this line to check the dimensions
...
#ACE_time1_time2[[paste0('ACEcat_extended_',timeperiod[1],'_',timeperiod[2],'yrs')]]<-
 # cut(ACE_time1_time2[[paste0('ACEscore_extended_',timeperiod[1],'_',timeperiod[2],'yrs')]],
  #    breaks=c(-1,1,2,5,22),labels=c('0_1low','2lomid','3_5midhi','6+high'))
#classic ACEs
...
#      breaks=c(-1,0,1,3,22),labels=c('0','1','2_3','4+'))
...
ncol(alspacKids_ACE_data_2018)#Should include 304 variables
...
write.dta(alspacKids_ACE_data_2018,
          file=paste0(loc_out,fileid_out,"ALL2.dta"))
